...
public partial class NewUnitTest
{
    public static void SplitTest()
    {
        ...
        db.Queryable<SplitTestTable>()
            ...
            .SplitTable(tas => tas.Take(3))
            .ToPageList(1, 2, ref count);

        var table2019 = db.SplitHelper<SplitTestTable>().GetTableName("2019-12-1");
        db.Queryable<SplitTestTable>()
            .Where(it => it.Name.Contains("a"))
            .SplitTable(tas => tas.InTableNames(table2019))
            .ToList();

        db.Queryable<SplitTestTable>()
            .Where(it => it.Id == 1)
            .SplitTable(tas => tas.Where(y => y.TableName.Contains("2019")))
            .ToList();

        var deldata = new SplitTestTable() { Id = id, CreateTime = DateTime.Now };
        var tableName = db.SplitHelper(deldata).GetTableNames();
        db.Deleteable<SplitTestTable>()
            .Where(deldata)
            .SplitTable(tas => tas.InTableNames(tableName))
            .ExecuteCommand();
        ...
        db.CurrentConnectionConfig.ConfigureExternalServices.SplitTableService =
            new WordSplitService();
        db.Insertable(new WordTestTable() { CreateTime = DateTime.Now, Name = "BC" })
            .SplitTable()
            .ExecuteReturnSnowflakeId();
        db.Insertable(new WordTestTable() { CreateTime = DateTime.Now, Name = "AC" })
            .SplitTable()
            .ExecuteReturnSnowflakeId();
        db.Insertable(new WordTestTable() { CreateTime = DateTime.Now, Name = "ZBZ" })
            .SplitTable()
            .ExecuteReturnSnowflakeId();
...
public class WordSplitService : ISplitTableService
{
    ...
    /// <returns></returns>
    public List<SplitTableInfo> GetAllTables(
        ISqlSugarClient db,
